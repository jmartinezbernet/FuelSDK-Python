...
class ET_Client(object):
    ...
    def configure_client(self, get_server_wsdl, params, tokenResponse):
        ...
        if self.application_type in ["public", "web"]:
            if self.is_none_or_empty_or_blank(self.authorization_code) or self.is_none_or_empty_or_blank(
                self.redirect_URI
            ):
                raise Exception(
                    "authorizationCode or redirectURI is null: For Public/Web Apps, the authorizationCode and redirectURI must be "
        ...
        if self.application_type == "public":
            if self.is_none_or_empty_or_blank(self.client_id):
                raise Exception(
                    "clientid is null: clientid must be passed when instantiating ET_Client or must be provided in environment variables / config file"
                )
            ...
            if self.is_none_or_empty_or_blank(self.client_id) or self.is_none_or_empty_or_blank(self.client_secret):
                raise Exception(
                    "clientid or clientsecret is null: clientid and clientsecret must be passed when instantiating ET_Client "
    ...
    def load_wsdl(self, wsdl_url, wsdl_file_local_location, get_server_wsdl=False):
        ...
        if (
            not os.path.exists(file_location) or os.path.getsize(file_location) == 0
    ...
    def refresh_token_with_oAuth2(self, force_refresh=False):
        ...
        if (
            force_refresh
            or self.authToken is None
            or self.authTokenExpiration is not None
            and time.time() + 300 > self.authTokenExpiration
        ):
    ...
    def get_soap_endpoint(self):
        ...
        if "url" in cache_file_data and "timestamp" in cache_file_data and cache_file_data["timestamp"] > time.time():
        ...
        try:
            r = requests.get(
                self.base_api_url + "/platform/v1/endpoints/soap",
                headers={"user-agent": "FuelSDK-Python-v1.3.0", "authorization": "Bearer " + self.authToken},
            )
